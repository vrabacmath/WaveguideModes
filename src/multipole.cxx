...
Multipole::Multipole(int N_multipole, int N_gauss, double multipole_radius, double point_defect_radius, double line_defect_radius, cpxd v, cpxd v_b, double delta)
    : N_multipole(N_multipole), N_gauss(N_gauss), multipole_radius(multipole_radius), point_defect_radius(point_defect_radius),
      line_defect_radius(line_defect_radius), v(v), v_b(v_b), delta(delta) {
...
MatrixXcd Multipole::crystal_M_operator(cpxd omega) {
    ...
    Vector2d alpha(0., 0.);
    Utils::multipole_defect_A(crystal_A, A_D_map, A_defect_map, N, this->multipole_radius, this->point_defect_radius, omega / this->v, omega / this->v_b, alpha, this->delta);
...
MatrixXcd Multipole::crystal_line_M_operator(cpxd omega, double alpha_x) {
    ...
    const MatrixXcd identity = MatrixXcd::Identity(density_count, density_count);
    Vector2d alpha(0., 0.);
    Utils::multipole_defect_A(crystal_A, A_D_map, A_defect_map, N, this->multipole_radius, this->line_defect_radius, omega / this->v, omega / this->v_b, alpha, this->delta);
    for (int j = 0; j < this->N_gauss; j++) {
